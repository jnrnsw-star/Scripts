...
-- Мгновенный статус после парсинга
pcall(function()
    local g = Instance.new("ScreenGui")
    g.Name = "TSUM_ParseOK"
    g.ResetOnSpawn = false
    g.DisplayOrder = 998
    g.Parent = LocalPlayer:WaitForChild("PlayerGui", 10)
    local t = Instance.new("TextLabel")
    t.Size = UDim2.fromOffset(220, 36)
    t.Position = UDim2.new(0.5, -110, 0, 8)
    t.BackgroundColor3 = Color3.fromRGB(20, 24, 32)
    t.BackgroundTransparency = 0.2
    t.TextColor3 = Color3.fromRGB(200, 210, 225)
    t.Font = Enum.Font.Gotham
    t.TextSize = 13
    t.Text = "TSUM — инициализация..."
    t.Parent = g
...
local function bootSplash(onContinue)
    local ok, err = pcall(function()
        ...
        btn.MouseButton1Click:Connect(function()
            btn.Active = false
            sub.Text = "Каталог вещей..."
            task.spawn(function()
                local okCat = pcall(ensureAutobuyCatalog)
                if not okCat then
                    warn("[TSUM] catalog load failed")
                end
                if gui.Parent then
                    gui:Destroy()
                end
...
--{{AUTOBUY_CATALOG_EMBEDDED}}
local AUTOBUY_ITEMS = nil
...
local function syncShopCatalogFromAutobuy()
    ...
    for rarity, list in pairs(AUTOBUY_ITEMS) do
        for _, it in ipairs(list) do
            if it.n and it.i then
                local id = tostring(it.i)
                SHOP_CATALOG.byName[it.n] = {
                    rarity = rarity,
                    fairPrice = it.p or 0,
                    spawnChance = it.s or 0,
                    id = id,
                    type = "",
                }
                SHOP_CATALOG.byId[id] = {
                    name = it.n,
                    rarity = rarity,
                    fairPrice = it.p or 0,
                    spawnChance = it.s or 0,
...
local function ensureAutobuyCatalog()
    if AUTOBUY_ITEMS and next(AUTOBUY_ITEMS) then
        return true
    end
    AUTOBUY_ITEMS = {}
    local compile = loadstring or load
    ...
    for _, rarity in ipairs({"Common","Uncommon","Rare","Epic","Legendary","Exclusive","TokyoExclusive"}) do
        local chunk = AUTOBUY_CATALOG_PARTS[rarity]
        if chunk then
            local fn, err = compile("return " .. chunk)
            if fn then
    ...
    if next(AUTOBUY_ITEMS) then
        syncShopCatalogFromAutobuy()
        return true
    end
    AUTOBUY_ITEMS = {
        ["Common"] = { { n = "Белая футболка", i = 1352050969, s = 55, p = 120 } },
        ["Rare"] = { { n = "Drip футболка", i = 6384915788, s = 8, p = 1200 } },
    }
    syncShopCatalogFromAutobuy()
    return false
end
...
local function loadMainUI()
    ...
    if not AUTOBUY_ITEMS or not next(AUTOBUY_ITEMS) then
        ensureAutobuyCatalog()
    end
    task.spawn(function()
